...
def show_ocr_upload():
    st.header("Uload Medical Report (OCR)")
    uploaded=st.file_uploader("Upload image(jpg, png, tiff). For bestFor best results crop to the report area.", type=["jpg","jpeg","png","tiff"])
    if uploaded is not None:
        ...
        if parsed:
            ...
            if st.button("Use extracted values & Predict"):
                st.session_state["input_values"] = {k.lower(): float(v) for k,v in parsed.items()}
